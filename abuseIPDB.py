    ...
    try:
        ...
        for ip in inputIPList:
            ...
            if ip:
                ...
                print("[{0}] Processing IP: {1}".format(
                    ipCounter, ip), end=' ... ')

                if validateIP(ip):
                    # make request to geoplugin
                    payload = {'key': APIKey, 'category': category,
        ...
        if len(failedIP) > 0:
            print("\nError occurred for following IPs [{0}]".format(
                len(failedIP)))
            for ip in failedIP:
                print(ip)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', '--APIKey', type=str, metavar='<AbuseIPDB API key>',
                        help='Enter AbuseIPDB API key', required=True)
    parser.add_argument('-f', '--inputfilename', type=str,
                        metavar='<input filename>', help='Input file name', required=True)
    ...
    if os.path.isfile(args.inputfilename) and os.access(args.inputfilename, os.R_OK):
        ...
        print("Error : either the file [{0}] does not exists or is not readable".format(
            args.inputfilename))
